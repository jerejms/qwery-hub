...
export async function getNextClassReminder(userId: string): Promise<NextClassInfo | null> {
  ...
  // Calculate minutes until each event and find the earliest one
  let nextEvent: ScheduleEvent | null = null;
  let minMinutesUntil: number | null = null;
  ...
  for (const event of allEvents) {
    ...
    // Track the event with the minimum time until
    if (minutesUntil !== null && (minMinutesUntil === null || minutesUntil < minMinutesUntil)) {
      minMinutesUntil = minutesUntil;
      nextEvent = event;
    }
  }

  // If no future event found this week, wrap around to next week
  if (!nextEvent) {
    ...
    minMinutesUntil = daysUntil * 24 * 60 + firstStartInt - currentTimeInt;
    nextEvent = firstEvent;
  }

  if (!nextEvent || minMinutesUntil === null) {
    return null;
  }
